...
from ryu.lib.packet import ipv4, ipv6, icmp
...
class ProjectController(app_manager.RyuApp):
    ...
    def get_link_cost(self, s1, s2):
        ...
        '''
        for x in range(5):
            self.send_ping_packet(s1, e1)
            self.send_ping_packet(s2, e2)
        
        return min(self.delay[int(s1.dpid)][int(e1.dpid)], self.delay[int(s2.dpid)][int(e2.dpid)])
